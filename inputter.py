...
def KeybdInputUnicode(ucode, isup):
    # https://docs.microsoft.com/en-us/windows/win32/api/winuser/ns-winuser-keybdinput
    if isup:
        return KEYBDINPUT(0, ucode, KEYEVENTF_KEYUP | KEYEVENTF_UNICODE, 0, None)
    return KEYBDINPUT(0, ucode, KEYEVENTF_UNICODE, 0, None)
...
def KeyboardUnicode(ucode, isup):
    return Input(KeybdInputUnicode(ucode, isup))
...
def keyboard_stream_unicode(string):
    for character in string.replace('\r\n', '\r').replace('\n', '\r'):
        code = ord(character)
        yield KeyboardUnicode(code, False)
        yield KeyboardUnicode(code, True)


################################################################################

def send_string(s, wait=0.05):
    for event in keyboard_stream_unicode(s):
        SendInput(event)
        time.sleep(wait)
